...
def add_or_extend_access(user_id, username, name, duration_days):
    with _lock:
        conn = _conn()
        row = conn.execute("SELECT expires_at FROM access WHERE user_id=?", (user_id,)).fetchone()
        now = now_utc()
        if row:
            try:
            ...
            conn.execute("UPDATE access SET username=?, name=?, expires_at=? WHERE user_id=?",
                         (username, name, iso(expires), user_id))
        else:
            expires = now + timedelta(days=duration_days)
            conn.execute("INSERT INTO access(user_id,username,name,expires_at,created_at) VALUES(?,?,?,?,?)",
...
def list_access():
    with _lock:
        conn = _conn()
        rows = conn.execute("SELECT * FROM access ORDER BY expires_at DESC").fetchall()
        conn.close()
        return [dict(r) for r in rows]

def save_group(chat_id, title=None, updated_by=None, **fields):
    allowed = {"rank_link", "rank_channel", "rank_post_id", "sub_channel"}
    with _lock:
        conn = _conn()
        row = conn.execute("SELECT * FROM groups WHERE chat_id=?", (chat_id,)).fetchone()
        data = dict(row) if row else {
            "chat_id": chat_id, "title": title or "", "rank_link": None,
            "rank_channel": None, "rank_post_id": None, "sub_channel": None,
        ...
        conn.execute("""
            INSERT INTO groups(chat_id,title,rank_link,rank_channel,rank_post_id,sub_channel,updated_by,updated_at)
            VALUES(?,?,?,?,?,?,?,?)
            ON CONFLICT(chat_id) DO UPDATE SET
            title=excluded.title, rank_link=excluded.rank_link,
            rank_channel=excluded.rank_channel, rank_post_id=excluded.rank_post_id,
            sub_channel=excluded.sub_channel, updated_by=excluded.updated_by,
            updated_at=excluded.updated_at
        """, (data["chat_id"], data["title"], data["rank_link"], data["rank_channel"],
              data["rank_post_id"], data["sub_channel"], data["updated_by"], data["updated_at"]))
        conn.commit()
        conn.close()
...
def find_seen_user(chat_id, username):
    ...
    with _lock:
        conn = _conn()
        row = conn.execute("""
            SELECT * FROM seen_users WHERE chat_id=? AND lower(username)=?
...
def add_force(chat_id, user_id, username, name):
    with _lock:
        conn = _conn()
        conn.execute("""
            INSERT INTO forced(chat_id,user_id,username,name,forced_at,rank_filled,subscribed)
            VALUES(?,?,?,?,?,0,0)
            ON CONFLICT(chat_id,user_id) DO UPDATE SET
...
def remove_force(chat_id, user_id):
    with _lock:
        conn = _conn()
        conn.execute("DELETE FROM forced WHERE chat_id=? AND user_id=?", (chat_id, user_id))
        conn.commit()
        conn.close()
...
def get_force(chat_id, user_id):
    with _lock:
        conn = _conn()
        row = conn.execute("SELECT * FROM forced WHERE chat_id=? AND user_id=?",
...
def list_forced(chat_id):
    with _lock:
        conn = _conn()
        rows = conn.execute("SELECT * FROM forced WHERE chat_id=? ORDER BY forced_at DESC",
                            (chat_id,)).fetchall()
        conn.close()
        return [dict(r) for r in rows]
...
def all_forced():
    with _lock:
        conn = _conn()
        rows = conn.execute("SELECT * FROM forced ORDER BY forced_at DESC").fetchall()
        conn.close()
        return [dict(r) for r in rows]
...
def set_rank_filled(chat_id, user_id, value=True):
    with _lock:
        conn = _conn()
        conn.execute("UPDATE forced SET rank_filled=? WHERE chat_id=? AND user_id=?",
...
def set_subscribed(chat_id, user_id, value=True):
    with _lock:
        conn = _conn()
        conn.execute("UPDATE forced SET subscribed=? WHERE chat_id=? AND user_id=?",
                     (1 if value else 0, chat_id, user_id))
        conn.commit()
        conn.close()

def find_access_user_by_username(username):
    username = username.lstrip("@").lower()
    with _lock:
        conn = _conn()
        row = conn.execute("""
